...
describe('RulesState', () => {
  ...
  test('RulesState is initialized', () => {
    expect(bundle.rules.snapshot[bundle.params.basePluginName]).toEqual(
      expect.any(Object)
    );
    expect(bundle.rules.snapshot['@typescript-eslint']).toEqual(
      expect.any(Object)
    );
  });
  ...
  test('Rules can be changed via events', () => {
    ...
    expect(bundle.rules.snapshot['xxx']['jest/consistent-test-it']).toEqual(
      expect.any(Object)
    );
  });
  ...
  test('no-mixed-operators cannot be processed', () => {
    ...
    expect(digest.canGUI).toBeFalse();
  ...
  test('Rules with an object that itself consists of an all-boolean object produce a key-value/multicheckbox', () => {
