...
const noGo = ['no-mixed-operators', 'react/jsx-no-script-url'];
...
export class RulesState extends NgxsDataRepository<RulesStateModel> {
  ...
  notYet(ruleName: string, rule: Rule): boolean {
    if (noGo.includes(ruleName)) return true;
    else {
  ...
  private makeKeyValue(scheme: any): GUIElement {
    if (
      ...
      let subType = rawType;
      if (rawType === 'boolean') subType = 'checkbox';
      else if (rawType === 'string') subType = 'text';
      return {
        subType: subType,
        type: 'key-value'
      };
  ...
  private makeKeyValueWithMultiselect(scheme: any): GUIElement {
    if (
      ...
      Object.values(scheme.properties).every(
        (value: any) => value.type === 'object'
      )
    ) {
      ...
      if (
        ...
        Object.values(firstObject.properties).every(
          (value: any) => value.type === 'boolean'
        )
      ) {
        ...
        if (
          keys.every(
            (key) => template === JSON.stringify(scheme.properties[key])
          )
        ) {
          return {
            keys: keys,
  ...
  private makeObject(scheme: any): GUIElement {
    ...
    if (/* (scheme.type === 'object') && */ scheme.properties) {
      ...
      const element: GUIElement = {
        elements: entries
          .map(([name, property]) =>
            this.makeSchemaDigestElement(property, name)
          )
          .filter((element) => !!element),
        type: 'object'
      };
  ...
  private makeStringArray(scheme: any): GUIElement {
    if (
      (scheme.type === 'array' || scheme.type?.[0]?.includes('array')) &&
  ...
  private normalize(eslintRules: RulesStateModel): RulesStateModel {
    ...
    Object.entries(model).forEach(
      ([_, rules]: [string, Record<string, Rule>]) => {
        Object.entries(rules)
          // NOTE: take care of obvious data noise case
          .map(([_, rule]: [string, Rule]) => {
            if (this.utils.isEmptyObject(rule.meta)) rule.meta = {} as any;
