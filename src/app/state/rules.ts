...
export class RulesState extends NgxsDataRepository<RulesStateModel> {
  ...
  changeRules(@Payload('changes') { pluginName, rules }): void {
  ...
  initialize(): void {
    ...
    window.addEventListener('message', event => {
      ...
      let model: RulesStateModel;
      switch (message.command) {
        case 'rules':
          model = Object.entries(message.rules)
            .reduce((acc, [ruleName, rule]) => {
              const [pluginName] = ruleName.split('/');
              let byPlugin = acc[pluginName];
              if (!byPlugin) 
                byPlugin = acc[pluginName] = { };
              byPlugin[ruleName] = rule;
              return acc;
            }, { });
          // now we are organized like our model
          Object.entries(model)
            .forEach(([pluginName, rules]) => this.changeRules({ pluginName, rules }));
          break;
      }
    });
  }
