...
const slice = (ret, str) =>
  (r, _x, _y, rr): void => texpect(ret, str, r, rr);

const unshift = (ret, str) =>
  (r, s): void => texpect(ret, str, r, s);
...
const CASES = [
  ...
  [
    'slice(0, - 2)',
    a1,
    (array): any => array.slice(0, -2),
    slice([0], a3)
  ],
  [
    'slice(0, 1)',
    a1,
    (array): any => array.slice(0, 1),
    slice([0], a3)
  ],
  [
    'unshift()',
    a1,
    (array): any => array.unshift(),
    unshift(3, a1)
  ],
  [
    'unshift(- 1)',
...
test('concat', () => {
  ...
  const concated = new CommentArray('quux' as any).concat(
    'qux',
    parsed
  );

  is(stringify(concated, null, 2), `[
  "quux",
  "qux",
  // bar
  "bar",
