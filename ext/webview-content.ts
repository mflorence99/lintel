...
export function webviewContentFactory(context: vscode.ExtensionContext, 
  currentPanel: vscode.WebviewPanel, 
  fileCache: Record<string, string>, 
  filePattern: string): () => void {
  ...
  return (): void => {
    ...
    vscode.workspace.findFiles(filePattern, `**/{${ignoredDirectories.join(',')}}/**`)
      .then((uris: vscode.Uri[]) => {
        ...
        const delta = (Object.keys(fileCache).length !== Object.keys(eslintFiles).length) || !Object.keys(fileCache).every(fileName => {
          return eslintFiles[fileName] === fileCache[fileName];
        });

        if (delta) {
          Object.keys(eslintFiles).forEach(key => fileCache[key] = eslintFiles[key]);
