...
describe('resolve-extends', () => {
  ...
  test('Extension names are normalized', () => {
    expect(normalizeExtensionName('/this/that.js')).toEqual({ moduleName: '/this/that.js' });
    expect(normalizeExtensionName('./that.js')).toEqual({ moduleName: './that.js' });
    expect(normalizeExtensionName('eslint:all')).toEqual({ moduleName: 'eslint:all' });
    ...
    expect(normalizeExtensionName('eslint-config-google')).toEqual({ moduleName: 'eslint-config-google' });
    expect(normalizeExtensionName('google')).toEqual({ moduleName: 'eslint-config-google' });
    expect(normalizeExtensionName('@google')).toEqual({ moduleName: '@google/eslint-config' });
    expect(normalizeExtensionName('@google/eslint-config-special')).toEqual({ moduleName: '@google/eslint-config-special' });
  });
  ...
  test('Extension can be required', () => {
    ...
    expect(result.extension).toBeTruthy();
    expect(result.modulePath.endsWith('node_modules/eslint-plugin-jest/lib/index.js')).toBe(true);
  ...
  test('Extension can be fully resolved', () => {
    ...
    extension = resolveExtension('plugin:@typescript-eslint/recommended-requiring-type-checking', __filename);
